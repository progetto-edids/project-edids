...
import com.nonsense.NonSenseGenerator.NonSenseGenerator;
import com.nonsense.Syntax.Syntax; // Necessario se Syntax viene passato
import com.nonsense.TemplateManager.TemplateManager;
import com.nonsense.ToxicityValidator.ToxicityValidator;

// Aggiungiamo le dipendenze al costruttore della UI
public class UI {
    ...
    private final TemplateManager templateManager;
    private final BucketManager bucketManager;
    private final Syntax syntaxAnalyzer; // Aggiungiamo la dipendenza per Syntax
    private final ToxicityValidator toxicityValidator; 
    private final double TOXICITY_THRESHOLD = 0.5;

    // Costruttore: Ora riceve NonSenseGenerator, Syntax e (opzionalmente) Dictionary già inizializzati
        ...
        while (continueGenerating) {
            ...
            try {
                ...
                if (input2.equals("yes") || input2.equals("y")) {
                    ...
                    // Validazione della tossicità, se è tossica non la mostra altrimenti la mostra e chiede se la si vuole salvare nel bucket
                    // ATTENZIONE: ToxicityValidator.getToxicityScore() probabilmente ha bisogno del LanguageServiceClient!
                    // Dovresti modificarlo per ricevere anche lui il LanguageServiceClient.
                    double toxicity = toxicityValidator.getToxicityScore(nonsense); // Potrebbe aver bisogno del LanguageServiceClient
